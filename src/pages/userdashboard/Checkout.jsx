...
const Checkout = () => {
  ...
  const handleApplyPromo = () => {
    if (promoApplied) return alert("Promo code already applied!");

    let discountAmount = 0;

    if (promoCode === "PROMO10") {
      discountAmount = subtotal * 0.10; // 10% off
    } else if (promoCode === "FOOD50") {
      discountAmount = 50; // Flat Rs. 50 off
    } else {
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-400"
                  disabled={promoApplied}
                />
                <button
                  onClick={handleApplyPromo}
                  disabled={promoApplied}
                  className="px-6 bg-orange-500 text-white rounded-xl font-medium hover:bg-orange-600 disabled:opacity-50"
                >
                  Apply
                </button>
              </div>
              {promoApplied && (
                <p className="text-green-600 text-sm mt-2">Promo Applied ✔</p>
              )}
            </div>
              ...
              <label className="block border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-all">
                {preview ? (
                  <div className="space-y-3">
                    <img src={preview} className="mx-auto max-h-48 rounded-lg shadow-md" />
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-orange-500 to-pink-600 text-white font-bold py-4 rounded-xl hover:from-orange-600 hover:to-pink-700 transition-all flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
